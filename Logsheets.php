...
// Fetch user data
$sql = "SELECT Name, Notes, Membership, Start, End, Remaining, Terms, Months, Program, Monthly_Terms, Start_of_Term, End_of_Term, Days, Remaining_Days, Status FROM logsheet";
...
<head>
    ...
    <title>Logsheets</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" 
    integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link rel="stylesheet" href="Logsheets.css">
...
<div class="sidebar">
        ...
        <path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2zm9 1.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 0-1h-4a.5.5 0 0 0-.5.5M9 8a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 0-1h-4A.5.5 0 0 0 9 8m1 2.5a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 0-1h-3a.5.5 0 0 0-.5.5m-1 2C9 10.567 7.21 9 5 9c-2.086 0-3.8 1.398-3.984 3.181A1 1 0 0 0 2 13h6.96q.04-.245.04-.5M7 6a2 2 0 1 0-4 0 2 2 0 0 0 4 0"/>
            ...
            <li><a href="TrainerAcc.php" id="trainer-acc-tab"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-seam-fill" viewBox="0 0 16 16">
        <path fill-rule="evenodd" d="M15.528 2.973a.75.75 0 0 1 .472.696v8.662a.75.75 0 0 1-.472.696l-7.25 2.9a.75.75 0 0 1-.557 0l-7.25-2.9A.75.75 0 0 1 0 12.331V3.669a.75.75 0 0 1 .471-.696L7.443.184l.01-.003.268-.108a.75.75 0 0 1 .558 0l.269.108.01.003zM10.404 2 4.25 4.461 1.846 3.5 1 3.839v.4l6.5 2.6v7.922l.5.2.5-.2V6.84l6.5-2.6v-.4l-.846-.339L8 5.961 5.596 5l6.154-2.461z"/>
            ...
            <li><a href="#" id="transac-histo-tab"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-seam-fill" viewBox="0 0 16 16">
        <path fill-rule="evenodd" d="M15.528 2.973a.75.75 0 0 1 .472.696v8.662a.75.75 0 0 1-.472.696l-7.25 2.9a.75.75 0 0 1-.557 0l-7.25-2.9A.75.75 0 0 1 0 12.331V3.669a.75.75 0 0 1 .471-.696L7.443.184l.01-.003.268-.108a.75.75 0 0 1 .558 0l.269.108.01.003zM10.404 2 4.25 4.461 1.846 3.5 1 3.839v.4l6.5 2.6v7.922l.5.2.5-.2V6.84l6.5-2.6v-.4l-.846-.339L8 5.961 5.596 5l6.154-2.461z"/>
...
<?php while ($row = $result->fetch_assoc()): ?>
    <tr>
      ...
      <td><?= $row['Months'] ?></td>
      <td><?= $row['Program'] ?></td>
      <td><?= $row['Monthly_Terms'] ?></td>
      <td><?= $row['Start_of_Term'] ?></td>
      <td><?= $row['End_of_Term'] ?></td>
      <td><?= $row['Days'] ?></td>
    ...
    <form method="POST" class="d-inline">
      ...
      <input type="hidden" name="remaining" value="<?= $row['Remaining'] ?>">
      <input type="hidden" name="terms" value="<?= $row['Terms'] ?>">
      <input type="hidden" name="months" value="<?= $row['Months'] ?>">
      <input type="hidden" name="program" value="<?= $row['Program'] ?>">
      <input type="hidden" name="monthly_terms" value="<?= $row['Monthly_Terms'] ?>">
      <input type="hidden" name="start_of_term" value="<?= $row['Start_of_Term'] ?>">
      <input type="hidden" name="end_of_term" value="<?= $row['End_of_Term'] ?>">
      <input type="hidden" name="days" value="<?= $row['Days'] ?>">
      <input type="hidden" name="remaining_days" value="<?= $row['Remaining_Days'] ?>">
