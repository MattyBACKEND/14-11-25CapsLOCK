...
<div class="card" id="bookingCard">
  ...
  <select id="sessions">
    ...
    <option value="2">2 sessions - ₱1</option>
    <option value="5">5 sessions - ₱1</option>
  </select>
...
paypal.HostedButtons({
    ...
    onApprove: function(data) {
      ...
      const amount = sessions === 1 ? 1 : (sessions === 2 ? 1 : 1);
      ...
      .then(res => {
          if (res.status === "success") {
              ...
              // Redirect to TrainerBooking
              setTimeout(() => {
                  window.location.href =
                    "TrainerBooking.php?booking=success&booking_id=" + res.booking_id;
              }, 2000);
...
function countBookingsForSlot(ymd,time){ 
    return loadBookings().filter(b=>b.date===ymd && b.time===time).length; 
}
...
function renderSlots(){
    ...
    baseSlots.forEach(s=>{
        ...
        if(countBookingsForSlot(selected.date,s)>=slotCapacity){ 
...
function updateSummary(){ 
    ...
    let amount = sessions===1 ? 1 : (sessions===2 ? 1 : 1); 
...
const bookingData = new FormData();
bookingData.append("trainer_id", "<?php echo $trainer_id; ?>");
bookingData.append("date", selected.date);  // ⚠️ important
bookingData.append("time", selected.slot);  // ⚠️ important
bookingData.append("sessions", sessions);
bookingData.append("total_price", amount);


    fetch("save_booking.php", { method:"POST", body:bookingData })
    .then(r => r.json())
    .then(res => {
        if(res.status==="success"){
            alert("Booking successful! Booking ID: "+res.booking_id);
            window.location.href = "TrainerBooking.php?booking=success&booking_id=" + res.booking_id;
        } else {
            alert("Booking failed: " + res.msg);
        }
    });
